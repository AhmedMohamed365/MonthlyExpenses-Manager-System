...
public:
    ...
    filter();
    vector<Expense> filter_all(vector<Expense> v , bool is_category , bool is_day , bool is_month , bool is_year , bool is_amount , string cat = "0" , int day = -1 , int month = -1 , int year = -1 , float amount = -1);
    ~filter();
};
